#!/usr/bin/env python3
"""
Splash247 适配器 — P2 行业媒体来源适配器
==========================================

按《FPSO项目可用信息源使用手册》P2 要求实现：
...
功能:
  1. 访问 splash247.com 搜索页面，采集 FPSO 相关文章列表。
...
Usage:
  python crawler/adapters/splash247.py                 # 完整运行
  python crawler/adapters/splash247.py --dry-run       # 仅采集，不写入数据库
  python crawler/adapters/splash247.py --local-only    # 保存文件到本地，不写入 Supabase
  python crawler/adapters/splash247.py --test          # 自测
"""
...
log = logging.getLogger("splash247-adapter")
...
def run_adapter(dry_run: bool = False, local_only: bool = False):
    ...
    log.info("Splash247 Adapter — %s — %s", mode_str, TODAY)
    log.info("=" * 60)
...
def run_test():
    ...
    log.info("=" * 60)
    log.info("SELF-TEST: Splash247 Adapter")
    log.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Splash247 适配器 — P2 行业媒体来源适配器",
...
Examples:
  ...
  python crawler/adapters/splash247.py --test          # 自测
  python crawler/adapters/splash247.py --dry-run       # 仅采集，不写入数据库
  python crawler/adapters/splash247.py --local-only    # 仅本地保存
        """,
    )
