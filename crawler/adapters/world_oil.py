...
功能:
  1. 访问 worldoil.com 搜索页面，采集 FPSO 相关文章列表。
...
Usage:
  python crawler/adapters/world_oil.py                 # 完整运行
  python crawler/adapters/world_oil.py --dry-run       # 仅采集，不写入数据库
  python crawler/adapters/world_oil.py --local-only    # 保存文件到本地，不写入 Supabase
  python crawler/adapters/world_oil.py --test          # 自测
"""
...
log = logging.getLogger("worldoil-adapter")
...
def run_adapter(dry_run: bool = False, local_only: bool = False):
    ...
    log.info("World Oil Adapter — %s — %s", mode_str, TODAY)
    log.info("=" * 60)
...
def run_test():
    ...
    log.info("=" * 60)
    log.info("SELF-TEST: World Oil Adapter")
    log.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="World Oil 适配器 — P2 行业媒体来源适配器",
...
Examples:
  ...
  python crawler/adapters/world_oil.py --test          # 自测
  python crawler/adapters/world_oil.py --dry-run       # 仅采集，不写入数据库
  python crawler/adapters/world_oil.py --local-only    # 仅本地保存
        """,
    )
