#!/usr/bin/env python3
"""
Offshore Energy 适配器 — P1 行业媒体来源适配器
================================================
...
功能:
  1. 访问 offshore-energy.biz 搜索页面，采集 FPSO 相关文章列表。
...
Usage:
  python crawler/adapters/offshore_energy.py                 # 完整运行
  python crawler/adapters/offshore_energy.py --dry-run       # 仅采集，不写入数据库
  python crawler/adapters/offshore_energy.py --local-only    # 保存文件到本地，不写入 Supabase
  python crawler/adapters/offshore_energy.py --test          # 自测
"""
...
SOURCE_DOMAIN = "offshore-energy.biz"
...
log = logging.getLogger("offshore-energy-adapter")
...
def run_adapter(dry_run: bool = False, local_only: bool = False):
    """
    Run the Offshore Energy adapter.
    ...
    Returns:
        dict with keys: mode, total_articles, candidate_events, inserted, html_path.
    """
    log.info("=" * 60)
    mode_str = "DRY-RUN" if dry_run else ("LOCAL-ONLY" if local_only else "FULL")
    log.info("Offshore Energy Adapter — %s — %s", mode_str, TODAY)
    log.info("=" * 60)
...
def run_test():
    ...
    log.info("=" * 60)
    log.info("SELF-TEST: Offshore Energy Adapter")
    log.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Offshore Energy 适配器 — P1 行业媒体来源适配器",
...
Examples:
  ...
  python crawler/adapters/offshore_energy.py --test          # 自测
  python crawler/adapters/offshore_energy.py --dry-run       # 仅采集，不写入数据库
  python crawler/adapters/offshore_energy.py --local-only    # 仅本地保存
        """,
    )
